...
test.requestHooks(mockOnlyOneIdp)('should auto redirect to 3rd party IdP login page with basic Signing in message', async t => {
  await setup(t);

  const successRedirectPage = new BasePageObject(t);
  await t.expect(successRedirectPage.getFormTitle()).eql('Signing in');
...
test.requestHooks(mockOnlyOneIdpAppUser)('should auto redirect to 3rd party IdP login page with Signing in longer message', async t => {
  await setup(t);

  const successRedirectPage = new BasePageObject(t);
  await t.expect(successRedirectPage.getFormTitle()).eql('Signing in to Okta Administration as admin@idx.com');
